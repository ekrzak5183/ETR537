...
```{r}
df4 |>
  mutate(participation_proportion = var_a/var_b)
```
...
-   I know what a lot of these variables would mean, like sutdent id, course id, subject, semester, and section. Some variables I can assume. for example, q1, q2, q3, etc are probably the questions where participants could respond on a scale 1 to 5 where 1 might be "strongly disagree" and 5 means "strongly agree" or something similar. I'm not sure what the int, val, percomp, and tv variables mean though. my first thoughts would be integer, value, and television. I would love to rename the variables so their meaning is more clear. I would be curious to see if the question responses are correlated to course section. Perhaps some professors are doing a better job or facilitating conversation, delivering content, or organizing material, which could be analyzed and implemented into other classes.
...
-   I didn't realize at first that df5 and df6 shared 3 of the same students. I expected there to be 7 new rows with a lot more NA's. I notice that Vin has an NA in the number of stem classes taken because he was not listed in df5. It is nice to compare the data side by side. We could investigate if there is a connection between interest in stem and stem courses taken.
...
-   This data set has more data available than either of the two source data sets by themselves. I notice that there are 830 observations in the joined data set, while gradebook had 717, and survey had 662. That means that there must have been some observations that were not on both data sets.
...
-   We have a common variable called section, but the joined_data lists this as a character, while the survey data set lists it as a numerical value.
...
-   I notice that it gives insightful data like the number of unique entries for variables. This lets me see that there are 5 different subjects and 4 different semesters in this data set. There are also histrogram outputs which list the minimum, maximum, 1st, 2nd, and 3rd quartile for the quantitative variables. I wish it did not give the output in scientific notation though, especially in cases where it is e+00.
...
```{r}
#I remembered a lot about making sure strings are in quotation marks here, and calling the correct column. I'm not sure how to do this without selecting the subject column, since the program gives me an error whenever I don't call subject as a selected variable# 
data_to_explore |>
...
-   It seems like you would want to pick a number of bins that would show sufficient differences between data points without necessarily making a bar for every single number between 0 and 150. I chose 50 because it would give a bar average for every increment of 3 on the x axis. This shows some dips that weren't shown before like around x=8.
...
```{r}
ggplot(data_to_explore) +
  geom_histogram(aes(x = time_spent))+
  facet_wrap(~subject)
...
-   I notice that FrSca has the biggest spikes because they have a high number of students taking that course. I've curious about the multiple peaks and dips in the AnPha course. Maybe there is some incentive for students to get "nice" numbers of online time, like having a grade tied to getting at least 1 hour of online time per week or something.
-   Whoops! All the stuff in quotations was when I made a scatterplot instead of a histrogram "It seems like people who spend more time in the class tend to get higher scores almost without exception. all the lowest scores come from people who spent barely any time on the course. OcnA, AnPha, and FrSca all have a higher number of high time spent and middling score pairs. I'm wondering if these classes may be harder then the others or the professors grade more strictly, because people can spend more time working on them, but not necessarily get a higher grade."
...
-   The code seems to completely standard for a scatter plot. The only thing I wonder about it is if we could make it more informative by adding aesthetic choices like color. I also see that time spent is correlated to grade earned. There is a clear lack of data points where a student spent a long time on the course and also got a low score.
...
ggplot(data_to_explore) +
  ...
  ggtitle("How time spent on LMS relates to percentage earned")
...
-   A large portion of the students with low engagement and low grades where due to dropping out. Those students will obviously have less points earned because they did not complete as many assignments and did not have the same amount of time to engage with the LMS.
...
```{r}
data_to_explore |> 
  select(proportion_earned, time_spent_hours, q1, tv, q2) |> 
  correlate() |> 
  rearrange() |>
  shave() |>
  fashion()
...
```{r}
m2 <- lm(proportion_earned ~ q1 + tv, data = data_to_explore)
...
-   I see that the p value is less than 0.05 so the answer to question 1 and tv would be significantly significant. It is hard to interpret these without know what the question was actually asking and without knowing what TV means. It's probably not television
