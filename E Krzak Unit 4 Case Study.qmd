...
-   **The raw data is pretty unintelligible from the str() command. When I did the str() command I got so many rows returned that I could hardly scroll past it. The glimpse function was very helpful to see all the different variables being tracked. The is so much data stored about things that I don't think about but need to be tracked for the site to work as expected. For example, I didn't really think about how the site needs to track the number of replies in order to display the reply count accurately in the user interface. I see that they also have the location of the sourse tweet tracked, or potentially they are tracking locations mentioned in the tweet.**
...
-   **The number of observations went from 4,258 to 131,232 observations in the tokens data set. This is because each tweet could have upwards of 280 characters if I remember correctly, so each tweet will be broken up into many new rows.**
...
-   **Many of the top terms seem like what you'd expect. Many people are talking about code, data, and week, because this was a weekly data challenge. Thomas_mock shows up fairly high on the list because he is so pivotal to the community. I was interested to see fun so high up, because it is such an emotional word compared to all the mathematical and computer language terms.**
...
-   Fun was the most commonly used positive word. It can be hard to say if people were saying the activity of the week was fun or not fun, because both instances would have been counted here. In general, this output showed all the positively associated words and ordered them from most common to least common.
...
**Your Response Here: This table says that about 55% of the tweets had a positive term in them and about 45% of the tweets did not have a positive term in them.**
...
tt_tweets <- search_tweets("#tidytuesday", n = 500)
```
...
**Your Response: Thomas Mock has a significantly higher rate of interaction than anyone else. It is almost 4 times as much as the next runner up. I also notice that a lot of the higher interaction senders are community accounts rather than specific individuals such as R4DScommunity and WeAreRLadies.**
...
-   **Nodes that have more connections will be located closer to the center. We can see the Thomas Mock node very clearly, but it is so much bigger than they others that it is hard to spot other central nodes. The is one larger node that sticks out closer to the left of the graph. It seems like most of the nodes are really only interacting with a small number of central nodes. This might mean that people are responding to a tweet by a popular person, but not branching off to create more conversations with other posters.**
