...
-   I think total time online, time spent on assignments, and total number of discussion messages posted may be predictors of the final grade. These things may be related to the amount of effort students were putting into the class and the level of engagement with their peers.
...
-   They found that \# of forum postings, mail messages sent, and assessments completed were significant predictors. The assessments completed may not be helpful, because students who don't finish assessments are going to clearly get lower grades. It may be for reasons beyond ability though.
...
-   The console read out is much easier to examine because it provides some space between the different variables. It visually sorts the data into columns, unlike the log-data view which has data one after the other separated by commas. This makes the data hard to look at. The log data does seem to have some value that does not appear in the console view though. The first row has 0501 at the end which I do not see in the console view
...
-   There are a lot of variables that I'm not sure what they mean like Post_uv and Post_tv. Many students had NA responses to these variables so it must be some variable that would not apply to everyone. I think we would want to include cleaning in the wrangling step to omit the NA responses.
...
-   This would be untidy because the columns do not appear to be variables. In a tidy data set, the columns are titles after the variables.
